  ...
  function formatWord(guess: unknown): unknown {
    const letters = ['ا', 'أ', 'إ', 'ؤ', 'ر', 'ز', 'د', 'ذ', 'و'];
  ...
  const Keypad: FC<{usedkeys?: []}> = ({usedkeys}) => {
    ...
    return (
      ...
      {upperRow.map(l => <KeypadLetter key={l} letter={l}></KeypadLetter>)}
        {middleRow.map(l => <KeypadLetter key={l} letter={l}></KeypadLetter>)}
        {lowerLower.map(l => <KeypadLetter key={l} letter={l}></KeypadLetter>)}
      </section>
  ...
  const KeypadLetter: FC<{letter: string, color?: string}> = ({letter, color}) => {
